<template>
  <div class="p-3 m-2 shadow-lg card col-3" :class="{ hidden: toon.isHidden }">
    
    
      <div class="text-end">
        <!-- delete -->
        <span class="p-3 clickable" @click="confirmDelete">
          <svg
        ...
        <span v-if="toon.isHidden" class="clickable" @click="toggleToon">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
        ...
        <span v-else class="clickable" @click="toggleToon">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
      ...
      <h4 class="toonName"> <span v-html="toonIcon"></span> {{ toon.name }}</h4>
      <h6 class="mb-5">
...
export default {
  ...
  mounted() {
    ...
    this.sliderValue.value = [
      this.toonDetail.currentTuneUp,
      this.toonDetail.preferredTuneUp,
    ]
  },
  methods: {
    ...
    confirmDelete() {
      this.$swal
        .fire({
          title: "Remove the selected toon?",
        ...
        .then((result) => {
          if (result.isConfirmed) {
            ...
            this.$store.dispatch("removeToon", this.toon.name)
          }
        })
    },
    ...
    toggleToon() {
      console.log("toggle?")
      this.$store.dispatch("toggleToon", this.toon.name)
    },
  },
}
</script>
